...
def getCurie(name, biolink_class):
    ...
    url = f'https://name-resolution-sri.renci.org/lookup?string={name}&autocomplete=true&offset=0&limit=10&biolink_type=DiseaseOrPhenotypicFeature'
    success = False
    failedCounts = 0
...
current_list = pd.read_excel("contraindication_list_filled.xlsx")

for idx, row in tqdm(current_list.iterrows(), total=len(current_list)):
    if row['disease label'] == "Error":
        new_id, new_label = getCurie(row['disease list'], "DiseaseOrPhenotypicFeature")
        #print(f"New IDs: {new_id[0]}, {new_label[0]}")
        current_list.at[idx,'disease curie'] = new_id[0]
        current_list.at[idx,'disease label'] = new_label[0]
